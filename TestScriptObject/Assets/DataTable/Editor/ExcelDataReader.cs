...
{
    //Excel第1行对应数据表名字
    private const int excelNameRow = 0;
    //Excel第2行对应字段名称
    private const int keyNameRow = 1;
    //Excel第3行对应字段类型
    private const int keyTypeRow = 2;
    //Excel第4行对应字段描述
    private const int keyDescRow = 3;
    //Excel第5行及以后对应字段值
    private const int keyValueRow = 4;
    ...
    public static void ReadOneExcelToCode(string excelFullPath, string codeSavePath)
    {
        ...
        {
            Debug.LogError($"读取Excel失败，Excel标记失效 : {excelMediumData.className}");
            return;
        }
        if (excelMediumData.excelName =="Language")
        {
            return;
        }
        ...
        {
            Debug.LogError($"读取Excel失败，Excel不允许生成CSCode : {excelMediumData.className}");
            return;
        }
        ...
        {
            Debug.LogError($"解析Excel失败 : {excelMediumData.className}");
            return;
        }
        ...
        //类名
        string codeFileName = excelMediumData.className + "ExcelData";
        //写文件，生成CS类文件
        ...
        //
        Debug.Log($"生成Excel的CS成功 : {excelMediumData.className}");
    }
    ...
    public static void CreateOneExcelAsset(string excelFullPath, string assetSavePath)
    {
        ...
        if (excelMediumData.excelName == "Language")
        {
            GenerateLocalizationTable.CreateLanguageAsset(excelMediumData, assetSavePath);
            return;
        }
        ...
        string assetAssignmentName = excelMediumData.className + "AssetAssignment";
        ...
        {
            Debug.LogError($"创建Asset失败，未找到Asset生成类 : {excelMediumData.className}");
            return;
        }
        //反射获取方法
    ...
    private static ExcelMediumData CreateClassCodeByExcelPath(string excelFileFullPath)
    {
        ...
        //Excel名字
        string[] names = excelFileFullPath.Split('/');
        excelMediumData.excelName = names.Last().Split('.')[0];
        //Excel路径
        excelMediumData.path = excelFileFullPath;
        //当前遍历的行
        int curRowIndex = 0;
        //开始读取，按行遍历
        while (excelReader.Read())
        {
            ...
            switch (curRowIndex)
            {
                case excelNameRow:
                    ...
                    string specialSignStr = datas[0];
                    excelMediumData.className = specialSignStr;
                    break;
                case keyNameRow:
                    //数据名称行
        ...
        {
            Debug.Log("读取Excel成功:"+ excelMediumData.excelName);
    ...
    private static bool CheckExcelMediumData(ref ExcelMediumData mediumData)
    {
        ...
        if (string.IsNullOrEmpty(mediumData.className))
        {
            Debug.LogError("Excel类名为空");
            return false;
        }
        ...
        if (mediumData.excelName!="Language" && mediumData.propertyNameArray[0] != "id")
        {
